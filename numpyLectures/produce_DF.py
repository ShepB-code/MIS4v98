...
produce_df = pd.DataFrame(produce_dictionary, index=['Price per Unit', 'Quantity Sold', 'Total Sales'])

# Transpose the DataFrame to have produce as rows
produce_df = produce_df.T
...
# Find the produce with the highest total sales
highest_sales_produce = produce_df['Total Sales'].idxmax()
highest_sales_value = produce_df['Total Sales'].max()

# Find the produce with the lowest total sales
lowest_sales_produce = produce_df['Total Sales'].idxmin()
lowest_sales_value = produce_df['Total Sales'].min()

print(f"Highest: {highest_sales_produce} with ${highest_sales_value}")
print(f"Lowest: {lowest_sales_produce} with ${lowest_sales_value}")

print()
print("Question 2: Using 'loc', display the quantity and total sales for 'Orange' and 'Beets' (together)")
print()

print(produce_df.loc[['Orange', 'Beets'], :])

print()
print("Question 3: Using 'loc', display the total sales for 'Apples' through 'Lettuce'")
print()

print(produce_df.loc['Apples':'Lettuce', 'Total Sales'])

print()
print("Question 4: Using 'at', update the quantity sold for Apricots to 11,955 and total sales to 44,353.05")
print()

produce_df.at['Apricots', 'Quantity Sold'] = 44343.05

print()
print("Question 5: What is the average quantity sold across all products? print out only quantity sold")
print()

print("Average Quantity Sold:", produce_df.loc[:, 'Quantity Sold'].mean())
...
produce_df2 = produce_df[(produce_df.loc[:, 'Quantity Sold'] >= 11500) & (produce_df.loc[:, 'Quantity Sold'] <= 12000)]
print(produce_df2)
...
print("Total Sales:", produce_df2.loc[:, 'Total Sales'].sum())
print(produce_df2.loc[:, 'Total Sales'])
